...
This fork extends the upstream project with the following features and integrations:

- **Local-library-aware search**: External albums, tracks, and playlists are deduplicated against the local Navidrome library. Matching handles artist case differences and common featured-artist title variants.
- **Download deduplication**: Tracks that are already present locally are reused instead of being downloaded again, including external tracks that have been mapped to their Navidrome IDs.
- **Improved external cover art**: External album artwork is cached, honours the Subsonic `size` parameter, and can display a configurable visual indicator until the album is downloaded. See the `EXTERNAL_COVER_INDICATOR_*` settings in `.env.example`.
- **Optional Navidrome upload API**: When enabled with `USE_NAVIDROME_UPLOAD_API=true`, downloaded files are uploaded directly to the [`filipton/navidrome`](https://github.com/filipton/navidrome) fork and indexed immediately, avoiding the usual library-scan delay. Upload location and library ID are configurable.
- **More robust Subsonic compatibility**: `reportPlayback` accepts both legacy `id` and OpenSubsonic `mediaId` values, external song IDs are resolved to local IDs when possible, and missing lyrics responses are normalised for clients that do not handle Subsonic error 70.
- **Qobuz application credentials**: `QOBUZ_APP_ID` and `QOBUZ_APP_SECRET` can be supplied to avoid scraping Qobuz's web bundle for these values.
...
|----------|---------------------|-------------|------------------|
| [SquidWTF](https://squid.wtf/) (Qobuz, Tidal, Amazon Music, Deemix) | No | Source-dependent | Tidal and Deemix |
